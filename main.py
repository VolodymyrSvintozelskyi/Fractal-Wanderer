...
class Geometry:
    ...
    def init_geom(x1, y1, x2, y2, absorb_prob):
        root_square = Geometry.Square(x1, y1, x2, y2, absorb_prob=absorb_prob, zone=0)
        geom_obj_storage = [root_square] + Geometry.createChild(root_square, 1)
        return root_square, geom_obj_storage
    ...
    def createChild(parent, level = 0):
        ...
        for i in range(4):
            ...
            # Left or right
                ...
                y1 = parent.y1 + side / 3
                y2 = parent.y2 - side / 3
            newchild = Geometry.Square(x1, y1, x2, y2, absorb_prob=ABSORBTION_PROB, zone=level)
            parent.addPortal(newchild, i)
            objs += [newchild]
            objs += Geometry.createChild(newchild, level + 1)
        return objs
...
def myworker(root_square, STEP):
    ...
    hist_x.append(x)
    hist_y.append(y)
    return WorkerResult(not alive, i, square.zone, tar_x, tar_y, hist_x, hist_y)
